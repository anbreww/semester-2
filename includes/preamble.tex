...
\usepackage{a4wide} 	% kill less trees, use smaller margins
...
\usepackage{siunitx}      % pretty units : \unit[val]{dim}
...
\usepackage{enumerate}	% specify numbering \begin{enumerate}[A]

\usepackage{lscape}     % use landscape mode with \begin{landscape}
\usepackage{wrapfig}    % invoke Satan and wrap figures in text

\usepackage{hyperref}
\usepackage[all]{hypcap} % fixes references pointing to a figure's caption
...
\hypersetup{
    ...
    pdftitle={2nd Semester Thesis},    % title
    pdfauthor={Andrew Watson},     % author
    pdfsubject={Self-sustaining air hockey puck},   % subject of the document
    pdfcreator={Andrew Watson},   % creator of the document
    pdfproducer={EPFL}, % producer of the document
    pdfkeywords={Piezo, Film damping, simulation}, % list of keywords
